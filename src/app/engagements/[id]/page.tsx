      ...
      <div className="max-w-container mx-auto px-gutter py-7">
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="otg-card otg-card--ivory">
              <p className={sectionHead}>Deliverables</p>
              {engagement.deliverables.length === 0 ? (
                <p className="text-sm text-muted">No deliverables added yet.</p>
              ) : (
                <ul className="flex flex-col gap-2">
                  {engagement.deliverables.map(d => (
                    <li key={d.id} className="flex items-center justify-between text-sm">
                      <span className="font-semibold text-heading">{d.title}</span>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="otg-card otg-card--ivory">
              <p className={sectionHead}>Notes</p>
              {engagement.notes.length === 0 ? (
                <p className="text-sm text-muted">No notes yet.</p>
              ) : (
                <ul className="flex flex-col gap-3">
                  {engagement.notes.map(n => (
                    <li key={n.id} className="text-sm border-l-2 border-rich-green pl-3">
                      <p className="text-body leading-relaxed">{n.content}</p>
                      <p className="text-xs text-muted mt-1">{auDate(n.createdAt)}</p>
                    </li>
                  ))}
                </ul>
              )}
